...
# of the absolute competitor. After EDA I'm seeing two primary reasons.
# 1. I see some records where a competitor only entered the absolute, and they have no record for a regular weight division Possibly an IBJJF ruling or exception I'm unfamiliar with.
#    Dealing with these missing weights in a multitude of ways.
...
# For the cases where a competitor's name is spelled differently between their weight class entrance and their absolue entrance, I will split our competitior name into a First Name,
...
# of these spelling errors.
# Notes:
# 1. I explored using a fuzzy match solution, but this did not work well.
# 2. I validated that using the approach below was getting accurate results the vast majority of the time.
...
#Drop out the duplicates that were caused by joining on Names.
